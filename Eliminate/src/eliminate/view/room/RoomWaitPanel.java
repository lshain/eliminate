...
import eliminate.view.TableWaitRoomAdapter;
import eliminate.view.TableWaitRoomRenderer;
...
/**
 * The Class RoomWaitPanel.
 */
@SuppressWarnings("serial")
public class RoomWaitPanel extends JPanel implements ActionListener,
	...
	/** The exit_hover. */
	private String exit_hover = "media/image/all/exit_hover.png";
	...
	// 离开按钮
	/** The leave. */
	private String leave = "media/image/all/leave.png";

	/** The leave_down. */
	private String leave_down = "media/image/all/leave_hover.png";

	/** The leave_hover. */
	private String leave_hover = "media/image/all/leave_hover.png";
	...
	/** The player. */
	private Object[][] player = new String[][] { { "房主", "炮灰乙" },
			{ "玩家", "流氓丙" }, { "玩家", "土匪丁" }, { "玩家", "逗比戊" } };
	...
	/**
	 * Instantiates a new room wait panel.
	 */
	public RoomWaitPanel() {
		isRoomOwner = false;
		// 各个道具的金钱
		money_hint = 35;
		money_bomb = 35;
	 ...
	 *            the p
	 */
	public void initPlayer(String[][] p) {
		if (players != null) {
			remove(players);
		}
		this.player = p;
		if (player[0][1].equals(LoginController.getUser().getUserId())) {
			setRoomOwner(true);
			this.player = new Object[p.length][3];
			for (int i = 0; i < p.length; i++) {
				player[i][0] = p[i][0];
				player[i][1] = p[i][1];
				try {
					player[i][2] = MyButton.makeButton(kick, kick_down,
							kick_hover, kick_disable);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					player[i][2] = new JButton("踢人");
					e.printStackTrace();
				}
			}
			players = new MyTable(player, new String[] { "备注", "昵称", "踢人" });
		} else {
			players = new MyTable(player, new String[] { "备注", "昵称" });
			setRoomOwner(false);
		}
		...
		if (isRoomOwner) {
			players.getColumnModel().getColumn(1).setPreferredWidth(159);
			players.getColumnModel().getColumn(2).setPreferredWidth(50);
			TableWaitRoomAdapter adapter = new TableWaitRoomAdapter(players);
			players.addMouseListener(adapter);
			players.addMouseMotionListener(adapter);
			players.getColumnModel().getColumn(2)
					.setCellRenderer(new TableWaitRoomRenderer(adapter));
		} else {
			players.getColumnModel().getColumn(1).setPreferredWidth(209);
		}
		...
		players.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				System.out.println();
				if (row > 0 && column == 2) {
					String name = (String) (players.getValueAt(row, 1));
					if (isRoomOwner) {
						// 踢人
						players.repaint(players.getCellRect(row, column, false));
						LoginController.writeObject(new MutiPlayerGameMsg(
								name, 0));
						AudioPlayer.playButtonOne();
					}
					...
					if (column != 3 && column != 4) {
						// 如果点击的是自己哪一行
						String player_name = players.getValueAt(row, 1)
								.toString();
	...
	public void mouseExited(MouseEvent e) {
		...
		if (e.getSource() == hint_bt && hint_bt.isEnabled()) {
			...
			new FriendInviteDialog(MainFrame.getOwner(), true);
		}
	}
